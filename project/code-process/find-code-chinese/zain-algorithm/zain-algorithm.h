...
namespace zain {
    ...
    bool IncludeChinese(string s); //判断字符串中是否存在中文

    //算法实验相关函数

    int BigIntIsOneNubBig(string a, string b);  //大整数比较大小
    string BigIntAdd(string a, string b);  //大整数相加
    string BigIntMinus(string a, string b);  //大整数相减(第一个参数减第二个参数)(待开发...)

    void quickSortInt(int* arr, int left, int right);  //整形数组快速排序
