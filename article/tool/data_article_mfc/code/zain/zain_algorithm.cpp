...
//  cin >> s;
//  vector<string> files;
//  getFiles(s, files);  //获取指定路径下所有文件名和路径(包含子目录下的文件)
//  for(int i = 0; i < files.size(); i++) {
//    cout << files[i] << endl;
//  }

  //CreatePath("./新建   文本文档");  //创建文件夹(支持含子文件的文件夹)

  //cout << BigIntAdd("48915600", "1230001") << endl;  //大整数相加

//  cout << BigIntMinus("48915600", "1230001") << endl;  //大整数相加
...
/*
从文件读取UTF8编码文件所有内容存入Unicode编码string串
头文件:
  #include<fstream>
  #include<string>
  #include<vector>
...
返回值:
  储存读取到的所有内容
例子:
  vector<string> str = FileReadUTF8ToUnicodeString("./EcuIdS/ysId.txt");
  for(long long i = 0; i < str.size(); i++) cout << str[i] << endl;
*/
vector<string> FileReadUTF8ToUnicodeString(string fileNamePath) {
	char iFileNameC[10005];  //打开文件的路径参数需要char型
	vector<string> str;
	...
	while (getline(ifile, data)) {
		//cout << data << endl;
		str.push_back(UTF8StringToUnicodeString(data));
	}
	ifile.close();
	return str;
}
...
/*
将Unicode编码string串写入到UTF8编码指定文件
头文件:
  #include<fstream>
  #include<string>
  #include<iostream>
...
例子:
  无
*/
bool UnicodeToUTF8StringWriteToFile(string fileNamePath, string data, string mode) {
	char oFileNameC[10005];  //打开文件的路径参数需要char型
	//FileNameStringToChar(oFileNameC, fileNamePath);
    ...
    data = UnicodeStringToUTF8String(data);
...
bool MessageBoxUTF8String(string str) {
    MessageBoxA(NULL, UTF8StringToUnicodeString(str).c_str(), "zain", 0);
    return true;
}
